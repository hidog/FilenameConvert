...
void    Worker::rename( QString src, QString dst )
{
    ...
    for( int i = 0; i < list.size()/2; i++ )
    {
        ...
        // 8 bit
        //fprintf( fp, "ffmpeg -i \"%s\" -i \"%s\" -map 0:0 -map 0:1 -map 1:0 -vcodec hevc_nvenc -cq 29 -pix_fmt yuv420p -acodec copy -scodec copy -disposition:s:0 default \"./output/%s\"\n", 
          //  utf8_tc_str.c_str(), utf8_sub_str.c_str(), output_str.c_str() );
    }
...
void    Worker::rename( QString src, QString dst )
{
    ...
    for( auto& info : list )
    {
        ...
        fprintf( fp, "ffmpeg -i \"%s\" -map 0:1 -map 0:0 -vcodec hevc_nvenc -cq 30 -pix_fmt p010le -acodec copy \"./output/%s\"\n", 
                    utf8_tc_str.c_str(), utf8_tc_str.c_str() );
        // 8 bit
        //fprintf( fp, "ffmpeg -i \"%s\" -map 0:0 -map 0:1 -vcodec hevc_nvenc -cq 31 -pix_fmt yuv420p -acodec copy \"./output/%s\"\n", 
          //          utf8_tc_str.c_str(), utf8_tc_str.c_str() );
    }
